  ...
  return (
    <section id="about" className="border-b border-border">
      <div className="flex justify-between items-center px-6 py-4 md:px-16 md:py-5 border-b border-border">
        <span className="text-[10px] tracking-[0.3em] uppercase text-muted before:content-['//'] before:text-coral before:mr-2">Sobre mí</span>
        ...
        <RevealOnScroll className="px-6 py-8 md:px-12 md:py-12 flex flex-col gap-8" delay={100}>
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {stack.map(tag => (
                <span key={tag} className="text-[11px] tracking-[0.1em] px-2.5 py-1 border border-border text-muted hover:border-coral hover:text-cream transition-colors cursor-default">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div>
            <p className="text-[10px] tracking-[0.25em] uppercase text-coral mb-3">Contacto directo</p>
            <div className="flex">
              {([
                {
                  label: 'LinkedIn',
